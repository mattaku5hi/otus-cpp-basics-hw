...
uint32_t crc32Update(uint32_t octet, uint32_t crc)
{
  return crc32Table[((crc) ^ (octet)) & 0xff] ^ ((crc) >> 8);
}

uint32_t crc32Intermediate(const char *bytes, size_t bytesCount, uint32_t current)
{
  for(; bytesCount; --bytesCount, ++bytes)
  {
    current = crc32Update(static_cast<uint32_t>(*bytes), current);
  }

  return current;
}

uint32_t crc32Final(const char *bytes, size_t bytesCount, uint32_t current)
{
  return ~crc32Intermediate(bytes, bytesCount, current);
}
